...
int refresh_time = 10; //in milliseconds (close to 120FPS)
int speed=10; //speed of ball
...
void initSocket(){
    ...
    if (input=='1'){  //server
        ...
        serverAddr.sin_addr.s_addr = inet_addr(IPv4_server.c_str()); //convert const char to proper address for address struct (also does ntohl())
    ...
    while (gameStarted){
        ...
        if (input=='1'){ //server
            ...
            if (recv(clientSocket, (char*)&msg, sizeof(int), 0)>0){ //receives opponent paddle position
                ...
                this_thread::sleep_for(std::chrono::milliseconds(refresh_time));  //small delay to ensure all threads have halted
                system("cls");
...
//score system and reset positions
...
void input_handler(){
    if (kbhit()){ //wait until any key is pressed
        ...
        if (key==0 || key==224){ //extended key detected
            ...
            if (key==72){  //up arrow
                ...
                if (y<rows) y++; //upper bound of paddle position
            }
...
int main(){
    ...
    physics_init(); //initialize physics parameters
    system("cls"); //initial terminal screen clear
    //threads initialization
    
    thread t1(core); //playerinput+motion+render+score
    thread t2(rec_msg);       //receive message from other PC  

    t1.join();
    t2.join(); 
    //have to put recv() in separate thread since it is blocking function.
    //but in another thread, it can safely wait for data and go to sleep without affecting other functions
    //and wakeup when something arrives

    //send() can be put in the same thread since it has to send only small amts of data
    //but for large amts of data, or based on network congestion, the send() fn can get blocked for long times,
    //and game fps decreases
    //thus for high payloads, better to put in another thread
